...
namespace Utility.Library.Internal.Win32
{
    public static class NativeMethods
    {
        ...
        public const int SIZE_TITLE = 256;
        public const int SIZE_STATIC = 1024;
     
        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
        public struct STRINGBUFFERTITLE
        {
            [MarshalAs(UnmanagedType.ByValTStr, SizeConst = SIZE_STATIC)]
            public string szText;
        }

        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
        public struct STRINGBUFFERSTATIC
        {
            [MarshalAs(UnmanagedType.ByValTStr, SizeConst = SIZE_STATIC)]
            public string szText;
        }
        ...
        [DllImport("user32.dll", CharSet = CharSet.Auto)]
        public static extern int GetWindowText(IntPtr hWnd, out STRINGBUFFERTITLE ClassName, int nMaxCount);
        ...
        [SecurityCritical, SuppressUnmanagedCodeSecurity, DllImport("user32.dll", CharSet = CharSet.Auto)]
        public static extern IntPtr SendMessage(HandleRef hWnd, HeaderControlMessages msg, IntPtr wParam, ref HDITEM lParam);
       // [SecurityCritical, SuppressUnmanagedCodeSecurity, DllImport("user32.dll", CharSet = CharSet.Auto)]
       // public static extern void SendMessage(HandleRef hWnd, HeaderControlMessages msg, int wParam, ref HD_HITTESTINFO hti);
        [SecurityCritical, SuppressUnmanagedCodeSecurity, DllImport("user32.dll", CharSet = CharSet.Auto)]
        public static extern IntPtr SendMessage(HandleRef hWnd, HeaderControlMessages msg, IntPtr wParam, IntPtr lParam);

        [SecurityCritical, SuppressUnmanagedCodeSecurity, DllImport("user32.dll", CharSet = CharSet.Unicode)]
        public static extern IntPtr SendMessage(HandleRef hWnd, int msg, IntPtr wParam, string lParam);

        [DllImport("user32.dll", EntryPoint = "SendMessage", CharSet = CharSet.Auto)]
        public static extern IntPtr SendMessage2(HandleRef hWnd, int msg, IntPtr wParam, IntPtr lParam);
        ...
        [SecurityCritical, SuppressUnmanagedCodeSecurity, DllImport("user32", CharSet = CharSet.Unicode)]
        ...
        [DllImport("user32.dll", CharSet = CharSet.Auto)]
        public extern static IntPtr GetDlgItem(HandleRef hWnd, int nControlID);
        ...
        public static int GetWindowLong(HandleRef hWnd, WindowLongValue nIndex)
        {
            ...
            if (IntPtr.Size == 4)
            {
                result = (int)GetWindowLongPtr(hWnd, (int)nIndex);
                error = Marshal.GetLastWin32Error();
            }
            else
            {
                IntPtr resultPtr = GetWindowLongPtr(hWnd, (int)nIndex);
                error = Marshal.GetLastWin32Error();
        ...
        public static IntPtr SetWindowLong(HandleRef hWnd, WindowLongValue nIndex, IntPtr dwNewLong)
        {
            ...
            if (IntPtr.Size == 4)
            {
                int intResult = (int)SetWindowLongPtr(hWnd, (int)nIndex, dwNewLong);
                error = Marshal.GetLastWin32Error();
                result = new IntPtr(intResult);
            }
            else
            {
                result = SetWindowLongPtr(hWnd, (int)nIndex, dwNewLong);
                error = Marshal.GetLastWin32Error();
            }
            return result;
        }
        ...
        {
            string path = null;
            string name = string.Format(CultureInfo.CurrentCulture, @"AppEvents\Schemes\Apps\.Default\{0}\.current\", soundName);
        ...
        public static IntPtr GetHIcon(ImageSource source)
        {
            ...
            if (frame != null && frame.Decoder.Frames.Count > 0)
            {
                ...
                using(System.Drawing.Bitmap bitmap = new System.Drawing.Bitmap(width, height, stride,
