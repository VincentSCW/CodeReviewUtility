...
namespace Utility.Library.Misc
{
        ...
        where TPropertySource : INotifyPropertyChanged
    {
        ...
        {
            return this.RegisterHandler(expression, handler, this);
        }
        ...
        /// <param name="handler">The callback to invoke when the property has changed.</param>
        /// <returns>The object on which this method was invoked, to allow for multiple invocations chained together.</returns>
        internal PropertyObserver<TPropertySource> RegisterHandler(Expression<Func<TPropertySource, object>> expression, Action<TPropertySource> handler, IWeakEventListener receiver)
        {
            if (expression == null)
            ...
            if (propertySource != null)
            {
                ...
                PropertyChangedEventManager.AddListener(propertySource, receiver, propertyName);
            }
        ...
        {
            return this.UnregisterHandler(expression, this);
        }
        ...
        /// <param name="propertyName">A lambda expression like 'n => n.PropertyName'.</param>
        /// <returns>The object on which this method was invoked, to allow for multiple invocations chained together.</returns>
        internal PropertyObserver<TPropertySource> UnregisterHandler(Expression<Func<TPropertySource, object>> expression, IWeakEventListener receiver)
        {
            if (expression == null)
                throw new ArgumentNullException("expression");
            ...
            if (propertySource != null)
            {
                if (_propertyNameToHandlerMap.ContainsKey(propertyName))
                {
                    ...
                    PropertyChangedEventManager.RemoveListener(propertySource, receiver, propertyName);
                }
            }
        ...
        internal TPropertySource GetPropertySource()
        {
            try
            {
	...
	public sealed class PropertyCollectionObserver<TPropertySource> : IWeakEventListener
		where TPropertySource : INotifyPropertyChanged
	{
		private IEnumerable<PropertyObserver<TPropertySource>> observers;

		public PropertyCollectionObserver(IEnumerable<TPropertySource> source)
		{
			this.observers = source.Select(p => new PropertyObserver<TPropertySource>(p)).ToList();
		}

		public PropertyCollectionObserver<TPropertySource> RegisterHandler(Expression<Func<TPropertySource, object>> expression, Action<TPropertySource> handler)
        {
			foreach(var o in this.observers)
			{
				o.RegisterHandler(expression, handler, this);
			}

			return this;
		}

		public PropertyCollectionObserver<TPropertySource> UnregisterHandler(Expression<Func<TPropertySource, object>> expression)
        {
			foreach(var o in this.observers)
			{
                o.UnregisterHandler(expression, this);
			}
			return this;
		}
		...
		bool IWeakEventListener.ReceiveWeakEvent(Type managerType, object sender, EventArgs e)
		{
            ...
            {
                TPropertySource propertySource = (TPropertySource)sender;
                var other = this.observers.FirstOrDefault(p => p.GetPropertySource().Equals(propertySource));
                if (other != null)
                   return ((IWeakEventListener)other).ReceiveWeakEvent(managerType, sender, e);
            }
